...
export class UserService extends BaseService<User> {
    ...
    async list(dto: QueryUserDto, user: UserPlayground): Promise<any> {
        ...
        // Console user management is tenant-scoped whenever a tenant context
        // has been verified. The global root account may still inspect all
        // accounts, while a tenant administrator/member can only see members
        // of the selected tenant.
        let tenantUserIds: string[] | undefined;
        const requestedTenantId = dto.tenantId?.trim();
        if (requestedTenantId && !isEnabled(user.isRoot) && requestedTenantId !== user.tenantId) {
            throw HttpErrorFactory.forbidden(
                "Tenant filter is limited to the active tenant",
                BusinessCode.PERMISSION_DENIED,
            );
        }
        const scopedTenantId =
            requestedTenantId ?? (!isEnabled(user.isRoot) ? user.tenantId : undefined);
        if (scopedTenantId) {
            if (!isEnabled(user.isRoot) && user.tenantRoleCode !== "admin") {
                throw HttpErrorFactory.forbidden(
                    "Only the tenant administrator can list tenant users",
                    BusinessCode.PERMISSION_DENIED,
                );
            }
        ...
        // Add the platform-account boundary to every OR branch. A standalone
        // `{ isRoot: 0 }` branch would otherwise turn a keyword query into an
        // unfiltered query for all non-root accounts.
        ...
        if (tenantUserIds) {
            // Apply the membership boundary to every OR branch generated by
            // keyword/status filters. An impossible UUID keeps an empty
            // membership list from becoming an unscoped query.
            const ids = tenantUserIds.length
                ? tenantUserIds
                : ["00000000-0000-0000-0000-000000000000"];
            if (where.length === 0) {
                where.push({ id: In(ids), ...(isEnabled(user.isRoot) ? {} : { isRoot: 0 }) });
            } else {
                where.forEach((conditions) => {
                    if (Array.isArray(conditions)) {
                        conditions.forEach((condition) => {
        ...
        const normalizedWhere = where.flat();
        const result = await this.paginate(dto, {
            where: normalizedWhere.length > 0 ? normalizedWhere : undefined,
            relations: ["role"],
