...
import { ConsoleMcpToolError, ConsoleMcpToolsService } from "./console-mcp-tools.service";
...
export class ConsoleMcpRuntimeService {
    constructor(private readonly toolsService: ConsoleMcpToolsService) {}
    ...
    async handleHttp(req: Request, res: Response, parsedBody?: unknown): Promise<void> {
        ...
        const user = (req as Request & { user?: UserPlayground }).user;
        if (!user) {
            res.status(401).json(
                this.errorResponse(null, {
                    code: -32001,
        ...
        const responses = await Promise.all(
            messages.map((message) => this.dispatch(message, user)),
        );
    ...
    private async dispatch(
        ...
        try {
                ...
                case "initialize":
                    return this.successResponse(id, {
                        protocolVersion:
                            ...
                            "2024-11-05",
                        capabilities: { tools: {} },
                        serverInfo: { name: CONSOLE_MCP_SERVER_NAME, version: "1.0.0" },
                        instructions: SERVER_INSTRUCTIONS,
                    });
                case "notifications/initialized":
                    return this.successResponse(id, {});
                case "tools/list": {
                    const tools = this.toolsService.listToolsForUser(user).map((t) => ({
                        name: t.name,
            ...
            if (error instanceof ConsoleMcpToolError) {
                return this.errorResponse(id, {
                    code: error.code === "permission_denied" ? -32003 : -32603,
