...
The system SHALL present workspace paths ending in `.html` or `.htm` in an assistant reply as an actionable browser-preview control when the conversation uses the explicit BuildingAI embed mode. Other inline paths and direct OpenCode routes MUST retain their existing behavior.
...
- **THEN** the preview renders that file without exposing a local `file://` URL or BuildingAI bearer credential
