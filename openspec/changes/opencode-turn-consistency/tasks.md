...
- [ ] 1.1 Add failing database tests for turn-ID idempotency, one active turn per conversation, unique message links, terminal checks, and unique runtime-bound session mapping
- [ ] 1.2 Add the OpenCode turn entity, conversation session/runtime columns, exports, and module registration
- [ ] 1.3 Add and test an idempotent versioned migration for turn constraints/indexes plus namespaced OpenCode billing uniqueness
...
- [ ] 2.2 Add pure tests for client-command-only request hashing, credential-free execution/billing snapshots, runtime fingerprints, and API/log snapshot redaction
- [ ] 2.3 Implement canonical command hashing and snapshot construction, failing explicitly when attachment references or runtime configuration are invalid
...
- [ ] 3.1 Add adapter tests for operation deadlines, cancellation, `/session/status`, session update time, and bounded exact-message lookup
- [ ] 3.2 Implement the tested read APIs with error classification against the currently deployed OpenCode contract
- [ ] 3.3 Add adapter tests for automatic permission replies, question rejection, session abort, and stable prompt `messageID`
- [ ] 3.4 Implement the tested mutating OpenCode APIs with exact session/request targeting and operation-specific deadlines
