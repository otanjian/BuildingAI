...
Provide Bowi AI and managed OpenCode sessions with one discoverable, secure MCP contract for operating only the personal Todo data of a verified BuildingAI user.
...
#### Scenario: Authenticated BuildingAI user invokes Todo
- **WHEN** a first-party agent turn carries a short-lived invocation assertion for a logged-in BuildingAI user
...
- **WHEN** the supplied session does not map unambiguously to a verified BuildingAI user
