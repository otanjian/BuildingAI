...
Provide one predictable local lifecycle for the Doris database, its knowledge-hub frontend, and its read-only MCP endpoint so developers can start, inspect, and stop the complete Doris stack from the BuildingAI launcher.
...
The launcher MUST record and stop only Doris components started by the launcher, including the Docker Compose services, frontend process, and MCP process. Stopping Doris MUST NOT stop BuildingAI, OpenCode, SAP MCP, or externally managed containers.
