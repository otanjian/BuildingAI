...
Ensure OpenCode conversations executed inside the retained iframe are reflected in BuildingAI token
...
- **WHEN** an existing OpenCode session without iframe billing state is opened through BuildingAI
...
- **WHEN** multiple BuildingAI API instances trigger reconciliation at the same time
...
- **WHEN** a BuildingAI durable OpenCode turn already references the remote user message
