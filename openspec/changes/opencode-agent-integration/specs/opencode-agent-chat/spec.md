...
- **THEN** the system MUST NOT run the native BuildingAI tool-loop agent for that request
...
### Requirement: Conversation records stay in BuildingAI with session mapping

The system SHALL persist Agent conversations and messages in BuildingAI, SHALL map each BuildingAI conversation id one-to-one to an OpenCode session id, and SHALL make the full conversation history visible in the Agent workbench.

#### Scenario: New conversation creates mapped OpenCode session

- **WHEN** a user starts a new conversation with an OpenCode Agent
- **THEN** the system MUST create a BuildingAI conversation record
...
- **THEN** the system MUST show previously persisted assistant text, tool steps, and artifact metadata from BuildingAI storage
...
- **AND** one conversation MUST NOT read another conversation’s artifact directory through the BuildingAI artifact API
...
- **THEN** the dialog MUST expose a preview that loads the artifact through BuildingAI’s authenticated artifact endpoint in an iframe
