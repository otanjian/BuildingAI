# opencode-title-inline-workspace Specification
...
embedded conversation and SHALL persist that title to the mapped Bowi AI conversation.
...
- **AND** Bowi AI persists the generated title to the mapped conversation
- **AND** the agent history displays the generated title without a full-page refresh

#### Scenario: Existing title is preserved

- **WHEN** an OpenCode session reports a title for a Bowi AI conversation that already has a
  non-placeholder title
- **THEN** the system MUST preserve the existing Bowi AI title

#### Scenario: OpenCode is temporarily unavailable

- **WHEN** the OpenCode title cannot be read or generated
- **THEN** the Bowi AI conversation remains accessible with its current title
