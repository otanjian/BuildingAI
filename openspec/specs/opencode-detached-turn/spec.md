# opencode-detached-turn Specification

## Purpose
Ensures OpenCode agent turns keep running and persist into Bowi AI after the browser HTTP stream disconnects, and recovers OpenCode sessions that hang mid-turn or finish ahead of Bowi AI so conversations stay usable and history is not stuck on false Aborted/timeout states.
## Requirements
### Requirement: OpenCode turn outlives passive HTTP disconnect

For an OpenCode agent conversation turn, the system SHALL continue the remote OpenCode session work and SHALL persist the turn outcome to Bowi AI storage even when the initiating HTTP stream ends due to passive disconnect (refresh, tab close, network loss, or proxy dropping the SSE), unless the user explicitly stops the turn.
...
- **AND** MUST persist the eventual assistant result (or a true remote error) to Bowi AI after the turn settles
...
- **THEN** the system MUST show the persisted Bowi AI messages for that turn without requiring OpenCode history as the UI source of truth
...
- **AND** MUST mark the Bowi AI turn as stopped/aborted accordingly
...
When a detached OpenCode turn settles successfully or with a handled error/stop/timeout, the system SHALL persist Bowi AI messages for that turn before treating the turn as complete for clients (stream finish for live subscribers, and clearing in-flight status for reopen/list).
...
- **THEN** the system MUST still persist the turn messages to Bowi AI
...
The system SHALL allow at most one active server-owned OpenCode turn per Bowi AI conversation at a time.
...
When the server-owned turn hits its safety timeout, the system SHALL persist a timeout outcome to Bowi AI and SHALL best-effort abort the mapped OpenCode session so the remote session does not remain stuck mid-tool.
...
- **THEN** the system MUST mark the Bowi AI turn as timed out (or equivalent)
...
- **WHEN** the user reopens a conversation whose mapped OpenCode session still has unfinished mid-turn work (for example last assistant without finish) after Bowi AI already timed out or aborted
- **THEN** the system MUST attempt to abort that OpenCode session
- **AND** MUST clear the Bowi AI in-flight indicator for that conversation

#### Scenario: Thin-heal missing assistant from OpenCode

- **WHEN** OpenCode has a completed assistant turn for the conversation that Bowi AI lacks (or Bowi AI only stored a timeout/Aborted placeholder for that user turn)
- **THEN** on reopen or before the next send the system MUST best-effort persist a Bowi AI assistant message filled from that OpenCode turn (gap fill only)
