# opencode-embedded-experience-ux Specification
...
The OpenCode agent page SHALL omit the redundant Bowi AI conversation placeholder and SHALL
...
The Bowi AI-owned iframe header SHALL expose a project-files action for OpenCode agents.
...
Only when OpenCode is rendered with the explicit Bowi AI embed marker, its base background, text
hierarchy, border treatment, sans-serif and monospace fonts SHALL align with Bowi AI's embedded
...
- **WHEN** an OpenCode session loads with the Bowi AI embed marker
- **THEN** embed-only visual tokens and structured-part presentation are applied

#### Scenario: Bowi AI theme differs from the operating system

- **WHEN** Bowi AI is manually set to light or dark mode while the operating system uses the
  opposite color scheme
- **THEN** the existing iframe element communicates Bowi AI's resolved color scheme and the
  embedded surface uses the matching light or dark tokens without remounting

#### Scenario: Direct OpenCode is displayed

- **WHEN** the same OpenCode application is opened without the Bowi AI embed marker
