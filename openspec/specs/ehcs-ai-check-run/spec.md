# ehcs-ai-check-run Specification
...
`POST /check-runs/:runId/items/:ruleId/ingest` SHALL accept `assistantText` and optional `conversationId`, parse PRD §5.1 JSON, insert or update `ehcs-check_results`, and set item status to `done` or `failed`.
