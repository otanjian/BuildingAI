# opencode-token-billing Specification

## Purpose
TBD - created by archiving change opencode-token-billing. Update Purpose after archive.
## Requirements
...
- **WHEN** OpenCode emits one or more assistant `message.updated` events that include `tokens` for the active session during a Bowi AI turn
