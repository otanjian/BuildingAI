...
#### Scenario: Apply application branding hue

- **WHEN** a registered enterprise application dashboard is rendered
- **THEN** its primary, muted, and dark dashboard CSS variables derive from that application's
  configured branding hue

### Requirement: ehcs-ai exclusion

This specification SHALL NOT require changes to `extensions/ehcs-ai` dashboard implementation.

#### Scenario: Render the existing ehcs-ai dashboard

- **WHEN** the ehcs-ai dashboard is opened after enterprise dashboard templates are introduced
- **THEN** it continues using its existing dashboard implementation without adopting these template
  requirements
...
#### Scenario: Load dashboard overview data

- **WHEN** a V1 dashboard template requests its overview
- **THEN** it renders from the existing `DashboardOverview` response without requiring a
  `sceneMetrics` field
