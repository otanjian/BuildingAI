# opencode-embedded-ui-runtime-integrity Specification

## Purpose
Ensures the Bowi AI-managed OpenCode process serves the locally compatible embedded Web UI and
cannot be accepted as ready when its executable or browser assets have silently drifted.
## Requirements
### Requirement: Managed runtime proves the Bowi AI Web UI contract

The managed OpenCode runtime SHALL expose an explicit, stable Bowi AI Web UI contract marker in
the HTML it serves, and Bowi AI MUST require that marker in addition to the health response and
...
- **WHEN** the root Web UI HTML also carries the expected Bowi AI contract marker
...
- **WHEN** the root Web UI HTML does not carry the expected Bowi AI contract marker
...
Bowi AI MUST reject a managed OpenCode executable before launch when it does not contain the
...
- **WHEN** the selected OpenCode executable does not contain the Bowi AI contract marker
...
- **THEN** the resulting executable MUST contain the Bowi AI Web UI contract marker
...
- **WHEN** a build completes but its executable does not contain the Bowi AI contract marker
