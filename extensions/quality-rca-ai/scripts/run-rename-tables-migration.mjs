/**
 * Run QRCA 0.1.3 table rename migration (quality-rca-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter quality-rca-ai build:api`):
 *   node extensions/quality-rca-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'quality_rca_ai'
         ORDER BY table_name`,
    );
    console.log("quality_rca_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
