/**
 * Fix duplicate quality_rca_ai tables: drop empty quality-rca-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'quality_rca_ai' ORDER BY table_name`,
    );
    console.log("quality_rca_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "quality_rca_ai"."${table_name}"`,
        );
