...
export function ChatPanel({
    ...
    return (
        <div className="quality-rca-card quality-rca-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--quality-rca-border)",
                    display: "flex",
            ...
            <div className="quality-rca-chat-msgs">
                {showOpening ? (
                    <div className="quality-rca-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="quality-rca-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="quality-rca-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="quality-rca-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`quality-rca-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`quality-rca-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
