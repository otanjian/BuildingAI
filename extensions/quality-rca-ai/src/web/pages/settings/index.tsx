...
import { QRCA_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionQualityRcaAgentViaPlatform } from "../../lib/provision-quality-rca-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasQualityRcaAgent = agentOptions.some((a) => a.name === QRCA_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionQualityRcaAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${QRCA_PLATFORM_AGENT_NAME}」并注册 quality-rca-mcp`
                    : `已更新「${QRCA_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasQualityRcaAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasQualityRcaAgent, handleProvision]);

    if (loading || (provisioning && !hasQualityRcaAgent)) {
        return (
            <div className="quality-rca-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 QRCA 智能体（quality-rca-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="quality-rca-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasQualityRcaAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{QRCA_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                    ...
                    >
                        {provisioning ? "更新中…" : "更新 QRCA 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{QRCA_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 quality-rca-mcp 工具。
                </p>
            )}

            <div className="quality-rca-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="quality-rca-btn quality-rca-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 QRCA 智能体"}
