...
describe("parseCheckResponse", () => {
    it("parses fenced JSON", () => {
        const text = `Analysis done.\n\`\`\`json\n{"ruleId":"QA_001","anomalies":[]}\n\`\`\``;
        const result = parseCheckResponse(text, "QA_001");
    ...
    it("fails on invalid JSON", () => {
        const result = parseCheckResponse("not json", "QA_001");
    ...
    it("parses multiple anomalies", () => {
        ...
        const result = parseCheckResponse(JSON.stringify(payload), "QA_002");
        expect(result.ok).toBe(true);
        if (result.ok) {
