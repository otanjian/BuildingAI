...
} from "../../../db/seed-data/quality-rca-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isQualityRcaDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof QRCA_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class QualityRcaPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: QRCA_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: QRCA_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: QRCA_PLATFORM_AGENT_NAME,
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.QRCA_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure QRCA platform agent exists, register quality-rca-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: QRCA_PLATFORM_AGENT_NAME } });
        const created = !agent;
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = QRCA_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = QRCA_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = QRCA_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = QRCA_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = QRCA_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = QRCA_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = QRCA_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("quality-rca-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of QRCA_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("quality-rca-mcp") ||
