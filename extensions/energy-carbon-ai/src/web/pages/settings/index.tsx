...
import { ECO_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionEnergyCarbonAgentViaPlatform } from "../../lib/provision-energy-carbon-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasEnergyCarbonAgent = agentOptions.some((a) => a.name === ECO_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionEnergyCarbonAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${ECO_PLATFORM_AGENT_NAME}」并注册 energy-carbon-mcp`
                    : `已更新「${ECO_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasEnergyCarbonAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasEnergyCarbonAgent, handleProvision]);

    if (loading || (provisioning && !hasEnergyCarbonAgent)) {
        return (
            <div className="energy-carbon-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 ECO 智能体（energy-carbon-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="energy-carbon-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasEnergyCarbonAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{ECO_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="energy-carbon-btn energy-carbon-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 ECO 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{ECO_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 energy-carbon-mcp 工具。
                </p>
            )}

            <div className="energy-carbon-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="energy-carbon-btn energy-carbon-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 ECO 智能体"}
