...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="energy-carbon-modal-backdrop" onClick={onClose}>
            <div className="energy-carbon-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="energy-carbon-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`energy-carbon-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="energy-carbon-chat-msg ai">…</div>
                    )}
                </div>
                <div className="energy-carbon-chat-inp">
                    ...
                    <button type="button" className="energy-carbon-btn energy-carbon-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
