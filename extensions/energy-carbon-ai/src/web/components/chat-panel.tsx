...
export function ChatPanel({
    ...
    return (
        <div className="energy-carbon-card energy-carbon-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--energy-carbon-border)",
                    display: "flex",
            ...
            <div className="energy-carbon-chat-msgs">
                {showOpening ? (
                    <div className="energy-carbon-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="energy-carbon-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="energy-carbon-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="energy-carbon-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`energy-carbon-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`energy-carbon-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="energy-carbon-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
