...
import {
    ECO_ERP_MCP_NAME_HINTS,
    ECO_PLATFORM_AGENT_AVATAR,
    ECO_PLATFORM_AGENT_DESCRIPTION,
    ECO_PLATFORM_AGENT_MAX_STEPS,
    ECO_PLATFORM_AGENT_NAME,
    ECO_PLATFORM_AGENT_OPENING_QUESTIONS,
    ECO_PLATFORM_AGENT_OPENING_STATEMENT,
    ECO_PLATFORM_AGENT_QUICK_COMMANDS,
    ECO_PLATFORM_AGENT_ROLE_PROMPT,
} from "../../../db/seed-data/energy-carbon-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isEnergyCarbonDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof ECO_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class EnergyCarbonPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: ECO_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: ECO_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: ECO_PLATFORM_AGENT_NAME,
            description: ECO_PLATFORM_AGENT_DESCRIPTION,
            rolePrompt: ECO_PLATFORM_AGENT_ROLE_PROMPT,
            openingStatement: ECO_PLATFORM_AGENT_OPENING_STATEMENT,
            openingQuestions: ECO_PLATFORM_AGENT_OPENING_QUESTIONS,
            quickCommands: ECO_PLATFORM_AGENT_QUICK_COMMANDS,
            maxSteps: ECO_PLATFORM_AGENT_MAX_STEPS,
            mcpServers: [
                {
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.ECO_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure ECO platform agent exists, register energy-carbon-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: ECO_PLATFORM_AGENT_NAME } });
        const created = !agent;

        if (!agent) {
            agent = this.agentRepo.create({
                name: ECO_PLATFORM_AGENT_NAME,
                description: ECO_PLATFORM_AGENT_DESCRIPTION,
                createMode: "direct",
                avatar: ECO_PLATFORM_AGENT_AVATAR,
                createBy: userId,
            });
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = ECO_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = ECO_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = ECO_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = ECO_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = ECO_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = ECO_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = ECO_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("energy-carbon-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of ECO_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("energy-carbon-mcp") ||
