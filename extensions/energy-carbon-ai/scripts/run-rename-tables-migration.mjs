/**
 * Run ECO 0.1.3 table rename migration (energy-carbon-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter energy-carbon-ai build:api`):
 *   node extensions/energy-carbon-ai/scripts/run-rename-tables-migration.mjs
 */
...
const EXTENSION_ID = "energy-carbon-ai";
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'energy_carbon_ai'
         ORDER BY table_name`,
    );
    console.log("energy_carbon_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
