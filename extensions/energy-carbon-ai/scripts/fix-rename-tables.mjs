/**
 * Fix duplicate energy_carbon_ai tables: drop empty energy-carbon-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'energy_carbon_ai' ORDER BY table_name`,
    );
    console.log("energy_carbon_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "energy_carbon_ai"."${table_name}"`,
        );
