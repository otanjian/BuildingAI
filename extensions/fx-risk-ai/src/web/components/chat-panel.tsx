...
export function ChatPanel({
    ...
    return (
        <div className="fx-risk-card fx-risk-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--fx-risk-border)",
                    display: "flex",
            ...
            <div className="fx-risk-chat-msgs">
                {showOpening ? (
                    <div className="fx-risk-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="fx-risk-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="fx-risk-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="fx-risk-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`fx-risk-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`fx-risk-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="fx-risk-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
