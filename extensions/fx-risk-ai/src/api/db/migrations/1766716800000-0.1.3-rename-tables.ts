...
import { FXR_AI_SCHEMA, FXR_TABLE_RENAMES } from "../fx-risk-table-names";
...
async function rowCount(dataSource: DataSource, tableName: string): Promise<number> {
    const rows: { count: string }[] = await dataSource.query(
        `SELECT COUNT(*)::text AS count FROM "${FXR_AI_SCHEMA}"."${tableName}"`,
    );
    return Number(rows[0]?.count ?? 0);
}

/**
 * Rename FXR tables to `fx-risk-*` prefixed names (idempotent).
 * If synchronize created empty `fx-risk-*` tables alongside legacy names, drop the empty duplicates first.
 */
export async function up(dataSource: DataSource): Promise<void> {
    for (const [legacyName, tableName] of FXR_TABLE_RENAMES) {
        ...
        if (hasLegacy && hasCurrent) {
            ...
            if (legacyRows > 0 && currentRows === 0) {
                await dataSource.query(
                    `DROP TABLE "${FXR_AI_SCHEMA}"."${tableName}"`,
                );
            } else if (legacyRows === 0 && currentRows > 0) {
                await dataSource.query(
                    `DROP TABLE "${FXR_AI_SCHEMA}"."${legacyName}"`,
                );
                continue;
            } else if (legacyRows === 0 && currentRows === 0) {
                await dataSource.query(
        ...
        if (legacyStillExists && !currentStillExists) {
            await dataSource.query(
                `ALTER TABLE "${FXR_AI_SCHEMA}"."${legacyName}" RENAME TO "${tableName}"`,
            );
        }
    }
}
