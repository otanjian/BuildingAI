/**
 * Run FXR 0.1.3 table rename migration (fx-risk-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter fx-risk-ai build:api`):
 *   node extensions/fx-risk-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'fx_risk_ai'
         ORDER BY table_name`,
    );
    console.log("fx_risk_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
