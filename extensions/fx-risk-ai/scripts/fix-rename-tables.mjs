/**
 * Fix duplicate fx_risk_ai tables: drop empty fx-risk-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'fx_risk_ai' ORDER BY table_name`,
    );
    console.log("fx_risk_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "fx_risk_ai"."${table_name}"`,
        );
