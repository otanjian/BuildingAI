/**
 * Upsert TAX check rules catalog (35 rules, all disabled).
 *
 * Usage (from repo root, after `pnpm --filter tax-compliance-ai build:api`):
 *   node extensions/tax-compliance-ai/scripts/seed-check-rules.mjs
 */
...
const catalogSeeder = seeders.find((s) => s.name === "TaxComplianceCheckRulesCatalogSeeder");
if (!catalogSeeder) {
    console.error("TaxComplianceCheckRulesCatalogSeeder not found. Run: pnpm --filter tax-compliance-ai build:api");
    process.exit(1);
}
...
console.log("Done. Open TAX → 检查规则 to verify 35 rules (all disabled).");
