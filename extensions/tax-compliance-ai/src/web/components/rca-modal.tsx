...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="tax-compliance-modal-backdrop" onClick={onClose}>
            <div className="tax-compliance-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="tax-compliance-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`tax-compliance-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="tax-compliance-chat-msg ai">…</div>
                    )}
                </div>
                <div className="tax-compliance-chat-inp">
                    ...
                    <button type="button" className="tax-compliance-btn tax-compliance-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
