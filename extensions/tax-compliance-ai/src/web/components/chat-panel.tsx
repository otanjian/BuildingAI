...
export function ChatPanel({
    ...
    return (
        <div className="tax-compliance-card tax-compliance-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--tax-compliance-border)",
                    display: "flex",
            ...
            <div className="tax-compliance-chat-msgs">
                {showOpening ? (
                    <div className="tax-compliance-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="tax-compliance-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="tax-compliance-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="tax-compliance-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`tax-compliance-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`tax-compliance-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="tax-compliance-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
