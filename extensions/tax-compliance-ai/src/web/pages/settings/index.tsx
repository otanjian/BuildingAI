...
import { TAX_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionTaxComplianceAgentViaPlatform } from "../../lib/provision-tax-compliance-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasTaxComplianceAgent = agentOptions.some((a) => a.name === TAX_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionTaxComplianceAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${TAX_PLATFORM_AGENT_NAME}」并注册 tax-compliance-mcp`
                    : `已更新「${TAX_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasTaxComplianceAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasTaxComplianceAgent, handleProvision]);

    if (loading || (provisioning && !hasTaxComplianceAgent)) {
        return (
            <div className="tax-compliance-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 TAX 智能体（tax-compliance-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="tax-compliance-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasTaxComplianceAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{TAX_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                    ...
                    >
                        {provisioning ? "更新中…" : "更新 TAX 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{TAX_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 tax-compliance-mcp 工具。
                </p>
            )}

            <div className="tax-compliance-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="tax-compliance-btn tax-compliance-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 TAX 智能体"}
