...
import {
    TAX_ERP_MCP_NAME_HINTS,
    TAX_PLATFORM_AGENT_AVATAR,
    TAX_PLATFORM_AGENT_DESCRIPTION,
    TAX_PLATFORM_AGENT_MAX_STEPS,
    TAX_PLATFORM_AGENT_NAME,
    TAX_PLATFORM_AGENT_OPENING_QUESTIONS,
    TAX_PLATFORM_AGENT_OPENING_STATEMENT,
    TAX_PLATFORM_AGENT_QUICK_COMMANDS,
    TAX_PLATFORM_AGENT_ROLE_PROMPT,
} from "../../../db/seed-data/tax-compliance-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isTaxComplianceDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof TAX_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class TaxCompliancePlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: TAX_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: TAX_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: TAX_PLATFORM_AGENT_NAME,
            description: TAX_PLATFORM_AGENT_DESCRIPTION,
            rolePrompt: TAX_PLATFORM_AGENT_ROLE_PROMPT,
            openingStatement: TAX_PLATFORM_AGENT_OPENING_STATEMENT,
            openingQuestions: TAX_PLATFORM_AGENT_OPENING_QUESTIONS,
            quickCommands: TAX_PLATFORM_AGENT_QUICK_COMMANDS,
            maxSteps: TAX_PLATFORM_AGENT_MAX_STEPS,
            mcpServers: [
                {
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.TAX_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure TAX platform agent exists, register tax-compliance-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: TAX_PLATFORM_AGENT_NAME } });
        const created = !agent;

        if (!agent) {
            agent = this.agentRepo.create({
                name: TAX_PLATFORM_AGENT_NAME,
                description: TAX_PLATFORM_AGENT_DESCRIPTION,
                createMode: "direct",
                avatar: TAX_PLATFORM_AGENT_AVATAR,
                createBy: userId,
            });
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = TAX_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = TAX_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = TAX_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = TAX_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = TAX_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = TAX_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = TAX_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("tax-compliance-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of TAX_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("tax-compliance-mcp") ||
