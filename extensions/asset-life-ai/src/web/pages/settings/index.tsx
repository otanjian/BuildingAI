...
import { AST_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionAssetLifeAgentViaPlatform } from "../../lib/provision-asset-life-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasAssetLifeAgent = agentOptions.some((a) => a.name === AST_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionAssetLifeAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${AST_PLATFORM_AGENT_NAME}」并注册 asset-life-mcp`
                    : `已更新「${AST_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasAssetLifeAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasAssetLifeAgent, handleProvision]);

    if (loading || (provisioning && !hasAssetLifeAgent)) {
        return (
            <div className="asset-life-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 AST 智能体（asset-life-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="asset-life-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasAssetLifeAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{AST_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="asset-life-btn asset-life-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 AST 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{AST_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 asset-life-mcp 工具。
                </p>
            )}

            <div className="asset-life-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="asset-life-btn asset-life-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 AST 智能体"}
