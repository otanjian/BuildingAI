...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="asset-life-modal-backdrop" onClick={onClose}>
            <div className="asset-life-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="asset-life-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`asset-life-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="asset-life-chat-msg ai">…</div>
                    )}
                </div>
                <div className="asset-life-chat-inp">
                    ...
                    <button type="button" className="asset-life-btn asset-life-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
