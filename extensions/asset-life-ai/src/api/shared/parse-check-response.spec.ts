...
describe("parseCheckResponse", () => {
    it("parses fenced JSON", () => {
        const text = `Analysis done.\n\`\`\`json\n{"ruleId":"AST_001","anomalies":[]}\n\`\`\``;
        const result = parseCheckResponse(text, "AST_001");
    ...
    it("fails on invalid JSON", () => {
        const result = parseCheckResponse("not json", "AST_001");
    ...
    it("parses multiple anomalies", () => {
        ...
        const result = parseCheckResponse(JSON.stringify(payload), "AST_002");
        expect(result.ok).toBe(true);
        if (result.ok) {
