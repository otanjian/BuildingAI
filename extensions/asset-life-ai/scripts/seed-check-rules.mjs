/**
 * Upsert AST check rules catalog (35 rules, all disabled).
 *
 * Usage (from repo root, after `pnpm --filter asset-life-ai build:api`):
 *   node extensions/asset-life-ai/scripts/seed-check-rules.mjs
 */
...
const catalogSeeder = seeders.find((s) => s.name === "AssetLifeCheckRulesCatalogSeeder");
if (!catalogSeeder) {
    console.error("AssetLifeCheckRulesCatalogSeeder not found. Run: pnpm --filter asset-life-ai build:api");
    process.exit(1);
}
...
console.log("Done. Open AST → 检查规则 to verify 35 rules (all disabled).");
