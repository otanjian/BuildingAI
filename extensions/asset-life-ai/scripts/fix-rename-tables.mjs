/**
 * Fix duplicate asset_life_ai tables: drop empty asset-life-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'asset_life_ai' ORDER BY table_name`,
    );
    console.log("asset_life_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "asset_life_ai"."${table_name}"`,
        );
