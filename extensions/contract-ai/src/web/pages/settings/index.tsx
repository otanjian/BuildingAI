...
import { CTR_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionContractAgentViaPlatform } from "../../lib/provision-contract-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasContractAgent = agentOptions.some((a) => a.name === CTR_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionContractAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${CTR_PLATFORM_AGENT_NAME}」并注册 contract-mcp`
                    : `已更新「${CTR_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasContractAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasContractAgent, handleProvision]);

    if (loading || (provisioning && !hasContractAgent)) {
        return (
            <div className="contract-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 CTR 智能体（contract-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="contract-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasContractAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{CTR_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                    ...
                    >
                        {provisioning ? "更新中…" : "更新 CTR 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{CTR_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 contract-mcp 工具。
                </p>
            )}
