/**
 * Fix duplicate contract_ai tables: drop empty contract-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'contract_ai' ORDER BY table_name`,
    );
    console.log("contract_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "contract_ai"."${table_name}"`,
        );
