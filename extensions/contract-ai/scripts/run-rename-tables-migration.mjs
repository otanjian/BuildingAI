/**
 * Run CTR 0.1.3 table rename migration (contract-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter contract-ai build:api`):
 *   node extensions/contract-ai/scripts/run-rename-tables-migration.mjs
 */
...
const EXTENSION_ID = "contract-ai";
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'contract_ai'
         ORDER BY table_name`,
    );
    console.log("contract_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
