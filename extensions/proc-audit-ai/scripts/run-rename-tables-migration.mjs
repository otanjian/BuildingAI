/**
 * Run PROC 0.1.3 table rename migration (proc-audit-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter proc-audit-ai build:api`):
 *   node extensions/proc-audit-ai/scripts/run-rename-tables-migration.mjs
 */
...
const EXTENSION_ID = "proc-audit-ai";
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'proc_audit_ai'
         ORDER BY table_name`,
    );
    console.log("proc_audit_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
