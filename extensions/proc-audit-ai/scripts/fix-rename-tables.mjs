/**
 * Fix duplicate proc_audit_ai tables: drop empty proc-audit-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'proc_audit_ai' ORDER BY table_name`,
    );
    console.log("proc_audit_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "proc_audit_ai"."${table_name}"`,
        );
