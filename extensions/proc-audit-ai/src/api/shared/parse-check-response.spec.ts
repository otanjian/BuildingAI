...
describe("parseCheckResponse", () => {
    it("parses fenced JSON", () => {
        const text = `Analysis done.\n\`\`\`json\n{"ruleId":"PROC_001","anomalies":[]}\n\`\`\``;
        const result = parseCheckResponse(text, "PROC_001");
    ...
    it("fails on invalid JSON", () => {
        const result = parseCheckResponse("not json", "PROC_001");
    ...
    it("parses multiple anomalies", () => {
        ...
        const result = parseCheckResponse(JSON.stringify(payload), "PROC_002");
        expect(result.ok).toBe(true);
        if (result.ok) {
