...
import { PROC_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionProcAuditAgentViaPlatform } from "../../lib/provision-proc-audit-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasProcAuditAgent = agentOptions.some((a) => a.name === PROC_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionProcAuditAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${PROC_PLATFORM_AGENT_NAME}」并注册 proc-audit-mcp`
                    : `已更新「${PROC_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasProcAuditAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasProcAuditAgent, handleProvision]);

    if (loading || (provisioning && !hasProcAuditAgent)) {
        return (
            <div className="proc-audit-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 PROC 智能体（proc-audit-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="proc-audit-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasProcAuditAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{PROC_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="proc-audit-btn proc-audit-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 PROC 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{PROC_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 proc-audit-mcp 工具。
                </p>
            )}

            <div className="proc-audit-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="proc-audit-btn proc-audit-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 PROC 智能体"}
