...
export function ChatPanel({
    ...
    return (
        <div className="proc-audit-card proc-audit-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--proc-audit-border)",
                    display: "flex",
            ...
            <div className="proc-audit-chat-msgs">
                {showOpening ? (
                    <div className="proc-audit-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="proc-audit-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="proc-audit-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="proc-audit-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`proc-audit-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`proc-audit-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="proc-audit-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
