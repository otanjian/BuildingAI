...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="proc-audit-modal-backdrop" onClick={onClose}>
            <div className="proc-audit-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="proc-audit-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`proc-audit-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="proc-audit-chat-msg ai">…</div>
                    )}
                </div>
                <div className="proc-audit-chat-inp">
                    ...
                    <button type="button" className="proc-audit-btn proc-audit-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
