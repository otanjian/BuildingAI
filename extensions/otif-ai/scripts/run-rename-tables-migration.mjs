/**
 * Run OTIF 0.1.3 table rename migration (otif-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter otif-ai build:api`):
 *   node extensions/otif-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'otif_ai'
         ORDER BY table_name`,
    );
    console.log("otif_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
