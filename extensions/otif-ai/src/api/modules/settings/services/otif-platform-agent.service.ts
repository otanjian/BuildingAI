...
} from "../../../db/seed-data/otif-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isOtifDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof OTIF_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class OtifPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: OTIF_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: OTIF_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: OTIF_PLATFORM_AGENT_NAME,
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.OTIF_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure OTIF platform agent exists, register otif-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: OTIF_PLATFORM_AGENT_NAME } });
        const created = !agent;
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = OTIF_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = OTIF_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = OTIF_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = OTIF_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = OTIF_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = OTIF_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = OTIF_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("otif-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of OTIF_ERP_MCP_NAME_HINTS) {
