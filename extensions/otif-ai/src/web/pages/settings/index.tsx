...
import { OTIF_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionOtifAgentViaPlatform } from "../../lib/provision-otif-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasOtifAgent = agentOptions.some((a) => a.name === OTIF_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionOtifAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${OTIF_PLATFORM_AGENT_NAME}」并注册 otif-mcp`
                    : `已更新「${OTIF_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasOtifAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasOtifAgent, handleProvision]);

    if (loading || (provisioning && !hasOtifAgent)) {
        return (
            <div className="otif-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 OTIF 智能体（otif-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="otif-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasOtifAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{OTIF_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                    ...
                    >
                        {provisioning ? "更新中…" : "更新 OTIF 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{OTIF_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 otif-mcp 工具。
                </p>
            )}
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="otif-btn otif-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 OTIF 智能体"}
