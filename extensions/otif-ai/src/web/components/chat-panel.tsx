...
export function ChatPanel({
    ...
    return (
        <div className="otif-card otif-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--otif-border)",
                    display: "flex",
            ...
            <div className="otif-chat-msgs">
                {showOpening ? (
                    <div className="otif-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="otif-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="otif-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="otif-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`otif-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`otif-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="otif-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
