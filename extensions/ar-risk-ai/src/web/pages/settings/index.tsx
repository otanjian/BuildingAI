...
import { ARR_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionArRiskAgentViaPlatform } from "../../lib/provision-ar-risk-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasArRiskAgent = agentOptions.some((a) => a.name === ARR_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionArRiskAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${ARR_PLATFORM_AGENT_NAME}」并注册 ar-risk-mcp`
                    : `已更新「${ARR_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasArRiskAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasArRiskAgent, handleProvision]);

    if (loading || (provisioning && !hasArRiskAgent)) {
        return (
            <div className="ar-risk-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 ARR 智能体（ar-risk-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="ar-risk-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasArRiskAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{ARR_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="ar-risk-btn ar-risk-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 ARR 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{ARR_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 ar-risk-mcp 工具。
                </p>
            )}

            <div className="ar-risk-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="ar-risk-btn ar-risk-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 ARR 智能体"}
