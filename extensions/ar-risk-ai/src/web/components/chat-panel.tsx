...
export function ChatPanel({
    ...
    return (
        <div className="ar-risk-card ar-risk-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--ar-risk-border)",
                    display: "flex",
            ...
            <div className="ar-risk-chat-msgs">
                {showOpening ? (
                    <div className="ar-risk-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="ar-risk-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="ar-risk-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="ar-risk-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`ar-risk-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`ar-risk-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="ar-risk-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
