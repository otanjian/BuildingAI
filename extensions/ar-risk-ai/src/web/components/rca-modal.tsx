...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="ar-risk-modal-backdrop" onClick={onClose}>
            <div className="ar-risk-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="ar-risk-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`ar-risk-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="ar-risk-chat-msg ai">…</div>
                    )}
                </div>
                <div className="ar-risk-chat-inp">
                    ...
                    <button type="button" className="ar-risk-btn ar-risk-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
