...
import { ARR_AI_SCHEMA, ARR_TABLE_RENAMES } from "../ar-risk-table-names";
...
async function rowCount(dataSource: DataSource, tableName: string): Promise<number> {
    const rows: { count: string }[] = await dataSource.query(
        `SELECT COUNT(*)::text AS count FROM "${ARR_AI_SCHEMA}"."${tableName}"`,
    );
    return Number(rows[0]?.count ?? 0);
}

/**
 * Rename ARR tables to `ar-risk-*` prefixed names (idempotent).
 * If synchronize created empty `ar-risk-*` tables alongside legacy names, drop the empty duplicates first.
 */
export async function up(dataSource: DataSource): Promise<void> {
    for (const [legacyName, tableName] of ARR_TABLE_RENAMES) {
        ...
        if (hasLegacy && hasCurrent) {
            ...
            if (legacyRows > 0 && currentRows === 0) {
                await dataSource.query(
                    `DROP TABLE "${ARR_AI_SCHEMA}"."${tableName}"`,
                );
            } else if (legacyRows === 0 && currentRows > 0) {
                await dataSource.query(
                    `DROP TABLE "${ARR_AI_SCHEMA}"."${legacyName}"`,
                );
                continue;
            } else if (legacyRows === 0 && currentRows === 0) {
                await dataSource.query(
        ...
        if (legacyStillExists && !currentStillExists) {
            await dataSource.query(
                `ALTER TABLE "${ARR_AI_SCHEMA}"."${legacyName}" RENAME TO "${tableName}"`,
            );
        }
    }
}
