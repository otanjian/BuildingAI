...
} from "../../../db/seed-data/ar-risk-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isArRiskDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof ARR_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class ArRiskPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: ARR_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: ARR_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: ARR_PLATFORM_AGENT_NAME,
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.ARR_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure ARR platform agent exists, register ar-risk-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: ARR_PLATFORM_AGENT_NAME } });
        const created = !agent;
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = ARR_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = ARR_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = ARR_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = ARR_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = ARR_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = ARR_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = ARR_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("ar-risk-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of ARR_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("ar-risk-mcp") ||
