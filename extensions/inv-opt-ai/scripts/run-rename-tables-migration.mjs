/**
 * Run INVO 0.1.3 table rename migration (inv-opt-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter inv-opt-ai build:api`):
 *   node extensions/inv-opt-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'inv_opt_ai'
         ORDER BY table_name`,
    );
    console.log("inv_opt_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
