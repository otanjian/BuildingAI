/**
 * Fix duplicate inv_opt_ai tables: drop empty inv-opt-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'inv_opt_ai' ORDER BY table_name`,
    );
    console.log("inv_opt_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "inv_opt_ai"."${table_name}"`,
        );
