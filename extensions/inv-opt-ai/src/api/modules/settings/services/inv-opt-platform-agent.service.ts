...
} from "../../../db/seed-data/inv-opt-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isInvOptDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof INVO_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class InvOptPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: INVO_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: INVO_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: INVO_PLATFORM_AGENT_NAME,
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.INVO_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure INVO platform agent exists, register inv-opt-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: INVO_PLATFORM_AGENT_NAME } });
        const created = !agent;
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = INVO_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = INVO_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = INVO_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = INVO_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = INVO_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = INVO_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = INVO_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("bowi-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of INVO_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("bowi-mcp") || lower(name).endsWith("-mcp") ||
