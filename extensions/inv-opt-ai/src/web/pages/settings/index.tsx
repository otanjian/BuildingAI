...
import { INVO_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionInvOptAgentViaPlatform } from "../../lib/provision-inv-opt-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasInvOptAgent = agentOptions.some((a) => a.name === INVO_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionInvOptAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${INVO_PLATFORM_AGENT_NAME}」并注册 inv-opt-mcp`
                    : `已更新「${INVO_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasInvOptAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasInvOptAgent, handleProvision]);

    if (loading || (provisioning && !hasInvOptAgent)) {
        return (
            <div className="inv-opt-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 INVO 智能体（inv-opt-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="inv-opt-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasInvOptAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{INVO_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="inv-opt-btn inv-opt-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 INVO 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{INVO_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 inv-opt-mcp 工具。
                </p>
            )}

            <div className="inv-opt-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="inv-opt-btn inv-opt-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 INVO 智能体"}
