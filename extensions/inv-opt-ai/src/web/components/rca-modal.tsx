...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="inv-opt-modal-backdrop" onClick={onClose}>
            <div className="inv-opt-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="inv-opt-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`inv-opt-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="inv-opt-chat-msg ai">…</div>
                    )}
                </div>
                <div className="inv-opt-chat-inp">
                    ...
                    <button type="button" className="inv-opt-btn inv-opt-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
