...
export function ChatPanel({
    ...
    return (
        <div className="inv-opt-card inv-opt-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--inv-opt-border)",
                    display: "flex",
            ...
            <div className="inv-opt-chat-msgs">
                {showOpening ? (
                    <div className="inv-opt-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="inv-opt-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="inv-opt-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="inv-opt-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`inv-opt-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`inv-opt-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="inv-opt-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
