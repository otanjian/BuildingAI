/**
 * Fix duplicate mdm_quality_ai tables: drop empty mdm-quality-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'mdm_quality_ai' ORDER BY table_name`,
    );
    console.log("mdm_quality_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "mdm_quality_ai"."${table_name}"`,
        );
