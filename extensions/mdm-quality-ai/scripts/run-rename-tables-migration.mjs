/**
 * Run MDM 0.1.3 table rename migration (mdm-quality-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter mdm-quality-ai build:api`):
 *   node extensions/mdm-quality-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'mdm_quality_ai'
         ORDER BY table_name`,
    );
    console.log("mdm_quality_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
