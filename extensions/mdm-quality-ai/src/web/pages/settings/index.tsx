...
import { MDM_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionMdmQualityAgentViaPlatform } from "../../lib/provision-mdm-quality-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasMdmQualityAgent = agentOptions.some((a) => a.name === MDM_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionMdmQualityAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${MDM_PLATFORM_AGENT_NAME}」并注册 mdm-quality-mcp`
                    : `已更新「${MDM_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasMdmQualityAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasMdmQualityAgent, handleProvision]);

    if (loading || (provisioning && !hasMdmQualityAgent)) {
        return (
            <div className="mdm-quality-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 MDM 智能体（mdm-quality-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="mdm-quality-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasMdmQualityAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{MDM_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="mdm-quality-btn mdm-quality-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 MDM 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{MDM_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 mdm-quality-mcp 工具。
                </p>
            )}

            <div className="mdm-quality-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="mdm-quality-btn mdm-quality-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 MDM 智能体"}
