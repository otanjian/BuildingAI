...
export function ChatPanel({
    ...
    return (
        <div className="mdm-quality-card mdm-quality-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--mdm-quality-border)",
                    display: "flex",
            ...
            <div className="mdm-quality-chat-msgs">
                {showOpening ? (
                    <div className="mdm-quality-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="mdm-quality-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="mdm-quality-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="mdm-quality-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`mdm-quality-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`mdm-quality-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="mdm-quality-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
