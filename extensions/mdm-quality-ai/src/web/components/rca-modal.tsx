...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="mdm-quality-modal-backdrop" onClick={onClose}>
            <div className="mdm-quality-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="mdm-quality-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`mdm-quality-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="mdm-quality-chat-msg ai">…</div>
                    )}
                </div>
                <div className="mdm-quality-chat-inp">
                    ...
                    <button type="button" className="mdm-quality-btn mdm-quality-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
