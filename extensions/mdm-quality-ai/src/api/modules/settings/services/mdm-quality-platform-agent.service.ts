...
import {
    MDM_ERP_MCP_NAME_HINTS,
    MDM_PLATFORM_AGENT_AVATAR,
    MDM_PLATFORM_AGENT_DESCRIPTION,
    MDM_PLATFORM_AGENT_MAX_STEPS,
    MDM_PLATFORM_AGENT_NAME,
    MDM_PLATFORM_AGENT_OPENING_QUESTIONS,
    MDM_PLATFORM_AGENT_OPENING_STATEMENT,
    MDM_PLATFORM_AGENT_QUICK_COMMANDS,
    MDM_PLATFORM_AGENT_ROLE_PROMPT,
} from "../../../db/seed-data/mdm-quality-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isMdmQualityDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof MDM_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class MdmQualityPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: MDM_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: MDM_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: MDM_PLATFORM_AGENT_NAME,
            description: MDM_PLATFORM_AGENT_DESCRIPTION,
            rolePrompt: MDM_PLATFORM_AGENT_ROLE_PROMPT,
            openingStatement: MDM_PLATFORM_AGENT_OPENING_STATEMENT,
            openingQuestions: MDM_PLATFORM_AGENT_OPENING_QUESTIONS,
            quickCommands: MDM_PLATFORM_AGENT_QUICK_COMMANDS,
            maxSteps: MDM_PLATFORM_AGENT_MAX_STEPS,
            mcpServers: [
                {
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.MDM_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure MDM platform agent exists, register mdm-quality-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: MDM_PLATFORM_AGENT_NAME } });
        const created = !agent;

        if (!agent) {
            agent = this.agentRepo.create({
                name: MDM_PLATFORM_AGENT_NAME,
                description: MDM_PLATFORM_AGENT_DESCRIPTION,
                createMode: "direct",
                avatar: MDM_PLATFORM_AGENT_AVATAR,
                createBy: userId,
            });
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = MDM_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = MDM_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = MDM_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = MDM_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = MDM_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = MDM_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = MDM_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("mdm-quality-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of MDM_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("mdm-quality-mcp") ||
