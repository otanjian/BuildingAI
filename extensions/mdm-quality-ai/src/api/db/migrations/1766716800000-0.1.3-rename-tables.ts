...
import { MDM_AI_SCHEMA, MDM_TABLE_RENAMES } from "../mdm-quality-table-names";
...
async function rowCount(dataSource: DataSource, tableName: string): Promise<number> {
    const rows: { count: string }[] = await dataSource.query(
        `SELECT COUNT(*)::text AS count FROM "${MDM_AI_SCHEMA}"."${tableName}"`,
    );
    return Number(rows[0]?.count ?? 0);
}

/**
 * Rename MDM tables to `mdm-quality-*` prefixed names (idempotent).
 * If synchronize created empty `mdm-quality-*` tables alongside legacy names, drop the empty duplicates first.
 */
export async function up(dataSource: DataSource): Promise<void> {
    for (const [legacyName, tableName] of MDM_TABLE_RENAMES) {
        ...
        if (hasLegacy && hasCurrent) {
            ...
            if (legacyRows > 0 && currentRows === 0) {
                await dataSource.query(
                    `DROP TABLE "${MDM_AI_SCHEMA}"."${tableName}"`,
                );
            } else if (legacyRows === 0 && currentRows > 0) {
                await dataSource.query(
                    `DROP TABLE "${MDM_AI_SCHEMA}"."${legacyName}"`,
                );
                continue;
            } else if (legacyRows === 0 && currentRows === 0) {
                await dataSource.query(
        ...
        if (legacyStillExists && !currentStillExists) {
            await dataSource.query(
                `ALTER TABLE "${MDM_AI_SCHEMA}"."${legacyName}" RENAME TO "${tableName}"`,
            );
        }
    }
}
