/**
 * Run APO 0.1.3 table rename migration (ap-opt-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter ap-opt-ai build:api`):
 *   node extensions/ap-opt-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'ap_opt_ai'
         ORDER BY table_name`,
    );
    console.log("ap_opt_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
