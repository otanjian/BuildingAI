/**
 * Fix duplicate ap_opt_ai tables: drop empty ap-opt-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'ap_opt_ai' ORDER BY table_name`,
    );
    console.log("ap_opt_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "ap_opt_ai"."${table_name}"`,
        );
