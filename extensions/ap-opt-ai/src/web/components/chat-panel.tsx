...
export function ChatPanel({
    ...
    return (
        <div className="ap-opt-card ap-opt-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--ap-opt-border)",
                    display: "flex",
            ...
            <div className="ap-opt-chat-msgs">
                {showOpening ? (
                    <div className="ap-opt-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="ap-opt-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="ap-opt-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="ap-opt-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`ap-opt-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`ap-opt-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="ap-opt-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
