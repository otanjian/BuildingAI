...
import { APO_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionApOptAgentViaPlatform } from "../../lib/provision-ap-opt-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasApOptAgent = agentOptions.some((a) => a.name === APO_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionApOptAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${APO_PLATFORM_AGENT_NAME}」并注册 ap-opt-mcp`
                    : `已更新「${APO_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasApOptAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasApOptAgent, handleProvision]);

    if (loading || (provisioning && !hasApOptAgent)) {
        return (
            <div className="ap-opt-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 APO 智能体（ap-opt-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="ap-opt-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasApOptAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{APO_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="ap-opt-btn ap-opt-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 APO 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{APO_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 ap-opt-mcp 工具。
                </p>
            )}

            <div className="ap-opt-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="ap-opt-btn ap-opt-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 APO 智能体"}
