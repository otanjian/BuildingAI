/**
 * Run FCST 0.1.3 table rename migration (forecast-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter forecast-ai build:api`):
 *   node extensions/forecast-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'forecast_ai'
         ORDER BY table_name`,
    );
    console.log("forecast_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
