/**
 * Fix duplicate forecast_ai tables: drop empty forecast-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'forecast_ai' ORDER BY table_name`,
    );
    console.log("forecast_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "forecast_ai"."${table_name}"`,
        );
