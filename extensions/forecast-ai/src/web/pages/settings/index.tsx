...
import { FCST_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionForecastAgentViaPlatform } from "../../lib/provision-forecast-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasForecastAgent = agentOptions.some((a) => a.name === FCST_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionForecastAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${FCST_PLATFORM_AGENT_NAME}」并注册 forecast-mcp`
                    : `已更新「${FCST_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasForecastAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasForecastAgent, handleProvision]);

    if (loading || (provisioning && !hasForecastAgent)) {
        return (
            <div className="forecast-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 FCST 智能体（forecast-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="forecast-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasForecastAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{FCST_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                    ...
                    >
                        {provisioning ? "更新中…" : "更新 FCST 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{FCST_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 forecast-mcp 工具。
                </p>
            )}
