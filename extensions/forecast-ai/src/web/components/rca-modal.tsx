...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="forecast-modal-backdrop" onClick={onClose}>
            <div className="forecast-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="forecast-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`forecast-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="forecast-chat-msg ai">…</div>
                    )}
                </div>
                <div className="forecast-chat-inp">
                    ...
                    <button type="button" className="forecast-btn forecast-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
