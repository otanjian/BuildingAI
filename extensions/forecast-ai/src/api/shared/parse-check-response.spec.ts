...
describe("parseCheckResponse", () => {
    it("parses fenced JSON", () => {
        const text = `Analysis done.\n\`\`\`json\n{"ruleId":"FCST_001","anomalies":[]}\n\`\`\``;
        const result = parseCheckResponse(text, "FCST_001");
    ...
    it("fails on invalid JSON", () => {
        const result = parseCheckResponse("not json", "FCST_001");
    ...
    it("parses multiple anomalies", () => {
        ...
        const result = parseCheckResponse(JSON.stringify(payload), "FCST_002");
        expect(result.ok).toBe(true);
        if (result.ok) {
