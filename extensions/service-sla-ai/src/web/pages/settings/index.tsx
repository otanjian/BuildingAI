...
import { SLA_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionServiceSlaAgentViaPlatform } from "../../lib/provision-service-sla-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasServiceSlaAgent = agentOptions.some((a) => a.name === SLA_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionServiceSlaAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${SLA_PLATFORM_AGENT_NAME}」并注册 service-sla-mcp`
                    : `已更新「${SLA_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasServiceSlaAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasServiceSlaAgent, handleProvision]);

    if (loading || (provisioning && !hasServiceSlaAgent)) {
        return (
            <div className="service-sla-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 SLA 智能体（service-sla-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="service-sla-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasServiceSlaAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{SLA_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="service-sla-btn service-sla-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 SLA 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{SLA_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 service-sla-mcp 工具。
                </p>
            )}

            <div className="service-sla-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="service-sla-btn service-sla-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 SLA 智能体"}
