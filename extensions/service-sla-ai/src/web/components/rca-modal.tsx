...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="service-sla-modal-backdrop" onClick={onClose}>
            <div className="service-sla-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="service-sla-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`service-sla-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="service-sla-chat-msg ai">…</div>
                    )}
                </div>
                <div className="service-sla-chat-inp">
                    ...
                    <button type="button" className="service-sla-btn service-sla-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
