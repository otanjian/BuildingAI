/**
 * Fix duplicate service_sla_ai tables: drop empty service-sla-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'service_sla_ai' ORDER BY table_name`,
    );
    console.log("service_sla_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "service_sla_ai"."${table_name}"`,
        );
