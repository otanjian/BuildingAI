/**
 * Run SLA 0.1.3 table rename migration (service-sla-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter service-sla-ai build:api`):
 *   node extensions/service-sla-ai/scripts/run-rename-tables-migration.mjs
 */
...
const EXTENSION_ID = "service-sla-ai";
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'service_sla_ai'
         ORDER BY table_name`,
    );
    console.log("service_sla_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
