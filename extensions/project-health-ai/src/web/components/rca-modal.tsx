...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="project-health-modal-backdrop" onClick={onClose}>
            <div className="project-health-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="project-health-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`project-health-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="project-health-chat-msg ai">…</div>
                    )}
                </div>
                <div className="project-health-chat-inp">
                    ...
                    <button type="button" className="project-health-btn project-health-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
