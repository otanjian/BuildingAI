...
import { PRJ_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionProjectHealthAgentViaPlatform } from "../../lib/provision-project-health-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasProjectHealthAgent = agentOptions.some((a) => a.name === PRJ_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionProjectHealthAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${PRJ_PLATFORM_AGENT_NAME}」并注册 project-health-mcp`
                    : `已更新「${PRJ_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasProjectHealthAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasProjectHealthAgent, handleProvision]);

    if (loading || (provisioning && !hasProjectHealthAgent)) {
        return (
            <div className="project-health-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 PRJ 智能体（project-health-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="project-health-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasProjectHealthAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{PRJ_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="project-health-btn project-health-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 PRJ 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{PRJ_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 project-health-mcp 工具。
                </p>
            )}

            <div className="project-health-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="project-health-btn project-health-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 PRJ 智能体"}
