...
import { PRJ_AI_SCHEMA, PRJ_TABLE, PRJ_TABLE_LEGACY } from "../project-health-table-names";
...
async function resolveResultsTable(dataSource: DataSource): Promise<string> {
    const rows: { table_name: string }[] = await dataSource.query(
        ...
        LIMIT 1
        `,
        [PRJ_AI_SCHEMA, PRJ_TABLE_LEGACY.CHECK_RESULTS, PRJ_TABLE.CHECK_RESULTS],
    );
    return rows[0]?.table_name ?? PRJ_TABLE.CHECK_RESULTS;
}

export async function up(dataSource: DataSource): Promise<void> {
    const table = await resolveResultsTable(dataSource);
    await dataSource.query(`
        ALTER TABLE "${PRJ_AI_SCHEMA}"."${table}"
        ADD COLUMN IF NOT EXISTS "resolved_at" timestamp;
    `);
    await dataSource.query(`
        UPDATE "${PRJ_AI_SCHEMA}"."${table}"
