/**
 * Fix duplicate project_health_ai tables: drop empty project-health-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'project_health_ai' ORDER BY table_name`,
    );
    console.log("project_health_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "project_health_ai"."${table_name}"`,
        );
