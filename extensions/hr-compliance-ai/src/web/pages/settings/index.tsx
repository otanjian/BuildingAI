...
import { HRC_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionHrComplianceAgentViaPlatform } from "../../lib/provision-hr-compliance-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasHrComplianceAgent = agentOptions.some((a) => a.name === HRC_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionHrComplianceAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${HRC_PLATFORM_AGENT_NAME}」并注册 hr-compliance-mcp`
                    : `已更新「${HRC_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasHrComplianceAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasHrComplianceAgent, handleProvision]);

    if (loading || (provisioning && !hasHrComplianceAgent)) {
        return (
            <div className="hr-compliance-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 HRC 智能体（hr-compliance-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="hr-compliance-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasHrComplianceAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{HRC_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="hr-compliance-btn hr-compliance-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 HRC 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{HRC_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 hr-compliance-mcp 工具。
                </p>
            )}

            <div className="hr-compliance-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="hr-compliance-btn hr-compliance-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 HRC 智能体"}
