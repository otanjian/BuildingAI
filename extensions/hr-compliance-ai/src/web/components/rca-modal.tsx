...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="hr-compliance-modal-backdrop" onClick={onClose}>
            <div className="hr-compliance-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="hr-compliance-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`hr-compliance-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="hr-compliance-chat-msg ai">…</div>
                    )}
                </div>
                <div className="hr-compliance-chat-inp">
                    ...
                    <button type="button" className="hr-compliance-btn hr-compliance-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
