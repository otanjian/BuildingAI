...
export function ChatPanel({
    ...
    return (
        <div className="hr-compliance-card hr-compliance-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--hr-compliance-border)",
                    display: "flex",
            ...
            <div className="hr-compliance-chat-msgs">
                {showOpening ? (
                    <div className="hr-compliance-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="hr-compliance-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="hr-compliance-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="hr-compliance-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`hr-compliance-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`hr-compliance-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="hr-compliance-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
