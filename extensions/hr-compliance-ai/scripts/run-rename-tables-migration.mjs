/**
 * Run HRC 0.1.3 table rename migration (hr-compliance-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter hr-compliance-ai build:api`):
 *   node extensions/hr-compliance-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'hr_compliance_ai'
         ORDER BY table_name`,
    );
    console.log("hr_compliance_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
