/**
 * Fix duplicate hr_compliance_ai tables: drop empty hr-compliance-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'hr_compliance_ai' ORDER BY table_name`,
    );
    console.log("hr_compliance_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "hr_compliance_ai"."${table_name}"`,
        );
