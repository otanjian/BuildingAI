/**
 * Run BDG 0.1.3 table rename migration (budget-control-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter budget-control-ai build:api`):
 *   node extensions/budget-control-ai/scripts/run-rename-tables-migration.mjs
 */
...
const EXTENSION_ID = "budget-control-ai";
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'budget_control_ai'
         ORDER BY table_name`,
    );
    console.log("budget_control_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
