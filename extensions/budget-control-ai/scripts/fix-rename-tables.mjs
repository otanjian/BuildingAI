/**
 * Fix duplicate budget_control_ai tables: drop empty budget-control-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'budget_control_ai' ORDER BY table_name`,
    );
    console.log("budget_control_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "budget_control_ai"."${table_name}"`,
        );
