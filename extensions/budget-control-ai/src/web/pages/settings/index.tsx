...
import { BDG_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionBudgetControlAgentViaPlatform } from "../../lib/provision-budget-control-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasBudgetControlAgent = agentOptions.some((a) => a.name === BDG_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionBudgetControlAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${BDG_PLATFORM_AGENT_NAME}」并注册 budget-control-mcp`
                    : `已更新「${BDG_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasBudgetControlAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasBudgetControlAgent, handleProvision]);

    if (loading || (provisioning && !hasBudgetControlAgent)) {
        return (
            <div className="budget-control-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 BDG 智能体（budget-control-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="budget-control-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasBudgetControlAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{BDG_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                    ...
                    >
                        {provisioning ? "更新中…" : "更新 BDG 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{BDG_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 budget-control-mcp 工具。
                </p>
            )}

            <div className="budget-control-form-row">
                <label>智能体</label>
