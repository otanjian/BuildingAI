...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="budget-control-modal-backdrop" onClick={onClose}>
            <div className="budget-control-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="budget-control-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`budget-control-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="budget-control-chat-msg ai">…</div>
                    )}
                </div>
                <div className="budget-control-chat-inp">
                    ...
                    <button type="button" className="budget-control-btn budget-control-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
