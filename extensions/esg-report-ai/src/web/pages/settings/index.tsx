...
import { ESG_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionEsgReportAgentViaPlatform } from "../../lib/provision-esg-report-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasEsgReportAgent = agentOptions.some((a) => a.name === ESG_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionEsgReportAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${ESG_PLATFORM_AGENT_NAME}」并注册 esg-report-mcp`
                    : `已更新「${ESG_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasEsgReportAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasEsgReportAgent, handleProvision]);

    if (loading || (provisioning && !hasEsgReportAgent)) {
        return (
            <div className="esg-report-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 ESG 智能体（esg-report-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="esg-report-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasEsgReportAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{ESG_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="esg-report-btn esg-report-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 ESG 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{ESG_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 esg-report-mcp 工具。
                </p>
            )}

            <div className="esg-report-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="esg-report-btn esg-report-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 ESG 智能体"}
