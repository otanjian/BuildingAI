...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="esg-report-modal-backdrop" onClick={onClose}>
            <div className="esg-report-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="esg-report-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`esg-report-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="esg-report-chat-msg ai">…</div>
                    )}
                </div>
                <div className="esg-report-chat-inp">
                    ...
                    <button type="button" className="esg-report-btn esg-report-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
