...
import {
    ESG_ERP_MCP_NAME_HINTS,
    ESG_PLATFORM_AGENT_AVATAR,
    ESG_PLATFORM_AGENT_DESCRIPTION,
    ESG_PLATFORM_AGENT_MAX_STEPS,
    ESG_PLATFORM_AGENT_NAME,
    ESG_PLATFORM_AGENT_OPENING_QUESTIONS,
    ESG_PLATFORM_AGENT_OPENING_STATEMENT,
    ESG_PLATFORM_AGENT_QUICK_COMMANDS,
    ESG_PLATFORM_AGENT_ROLE_PROMPT,
} from "../../../db/seed-data/esg-report-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isEsgReportDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof ESG_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class EsgReportPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: ESG_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: ESG_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: ESG_PLATFORM_AGENT_NAME,
            description: ESG_PLATFORM_AGENT_DESCRIPTION,
            rolePrompt: ESG_PLATFORM_AGENT_ROLE_PROMPT,
            openingStatement: ESG_PLATFORM_AGENT_OPENING_STATEMENT,
            openingQuestions: ESG_PLATFORM_AGENT_OPENING_QUESTIONS,
            quickCommands: ESG_PLATFORM_AGENT_QUICK_COMMANDS,
            maxSteps: ESG_PLATFORM_AGENT_MAX_STEPS,
            mcpServers: [
                {
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.ESG_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure ESG platform agent exists, register esg-report-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: ESG_PLATFORM_AGENT_NAME } });
        const created = !agent;

        if (!agent) {
            agent = this.agentRepo.create({
                name: ESG_PLATFORM_AGENT_NAME,
                description: ESG_PLATFORM_AGENT_DESCRIPTION,
                createMode: "direct",
                avatar: ESG_PLATFORM_AGENT_AVATAR,
                createBy: userId,
            });
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = ESG_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = ESG_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = ESG_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = ESG_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = ESG_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = ESG_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = ESG_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("esg-report-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of ESG_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("esg-report-mcp") ||
