/**
 * Fix duplicate esg_report_ai tables: drop empty esg-report-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'esg_report_ai' ORDER BY table_name`,
    );
    console.log("esg_report_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "esg_report_ai"."${table_name}"`,
        );
