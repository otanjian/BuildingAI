/**
 * Run ESG 0.1.3 table rename migration (esg-report-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter esg-report-ai build:api`):
 *   node extensions/esg-report-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'esg_report_ai'
         ORDER BY table_name`,
    );
    console.log("esg_report_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
