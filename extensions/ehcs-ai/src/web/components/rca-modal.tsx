...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="ehcs-modal-backdrop" onClick={onClose}>
            <div className="ehcs-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="ehcs-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`ehcs-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="ehcs-chat-msg ai">…</div>
                    )}
                </div>
                <div className="ehcs-chat-inp">
                    ...
                    <button type="button" className="ehcs-btn ehcs-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
