...
import { EHCS_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionEhcsAgentViaPlatform } from "../../lib/provision-ehcs-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasEhcsAgent = agentOptions.some((a) => a.name === EHCS_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionEhcsAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${EHCS_PLATFORM_AGENT_NAME}」并注册 ehcs-mcp`
                    : `已更新「${EHCS_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasEhcsAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasEhcsAgent, handleProvision]);

    if (loading || (provisioning && !hasEhcsAgent)) {
        return (
            <div className="ehcs-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 EHCS 智能体（ehcs-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="ehcs-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasEhcsAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{EHCS_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="ehcs-btn ehcs-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 EHCS 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{EHCS_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 ehcs-mcp 工具。
                </p>
            )}

            <div className="ehcs-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="ehcs-btn ehcs-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 EHCS 智能体"}
