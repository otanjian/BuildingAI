...
} from "../../../db/seed-data/ehcs-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isEhcsDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof EHCS_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class EhcsPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: EHCS_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: EHCS_PLATFORM_AGENT_NAME } });
    ...
    /**
     * Ensure EHCS platform agent exists, register bowi-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: EHCS_PLATFORM_AGENT_NAME } });
        const created = !agent;
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = EHCS_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = EHCS_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = EHCS_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = EHCS_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = EHCS_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = EHCS_PLATFORM_AGENT_QUICK_COMMANDS;
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of EHCS_ERP_MCP_NAME_HINTS) {
