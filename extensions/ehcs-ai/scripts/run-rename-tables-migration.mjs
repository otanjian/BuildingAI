/**
 * Run EHCS 0.1.3 table rename migration (ehcs-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter ehcs-ai build:api`):
 *   node extensions/ehcs-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'ehcs_ai'
         ORDER BY table_name`,
    );
    console.log("ehcs_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
