/**
 * Fix duplicate channel_inv_ai tables: drop empty channel-inv-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'channel_inv_ai' ORDER BY table_name`,
    );
    console.log("channel_inv_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "channel_inv_ai"."${table_name}"`,
        );
