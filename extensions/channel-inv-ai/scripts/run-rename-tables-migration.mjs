/**
 * Run CHI 0.1.3 table rename migration (channel-inv-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter channel-inv-ai build:api`):
 *   node extensions/channel-inv-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'channel_inv_ai'
         ORDER BY table_name`,
    );
    console.log("channel_inv_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
