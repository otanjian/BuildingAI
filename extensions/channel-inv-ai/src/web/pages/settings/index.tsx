...
import { CHI_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionChannelInvAgentViaPlatform } from "../../lib/provision-channel-inv-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasChannelInvAgent = agentOptions.some((a) => a.name === CHI_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionChannelInvAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${CHI_PLATFORM_AGENT_NAME}」并注册 channel-inv-mcp`
                    : `已更新「${CHI_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasChannelInvAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasChannelInvAgent, handleProvision]);

    if (loading || (provisioning && !hasChannelInvAgent)) {
        return (
            <div className="channel-inv-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 CHI 智能体（channel-inv-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="channel-inv-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasChannelInvAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{CHI_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="channel-inv-btn channel-inv-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 CHI 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{CHI_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 channel-inv-mcp 工具。
                </p>
            )}

            <div className="channel-inv-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="channel-inv-btn channel-inv-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 CHI 智能体"}
