...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="channel-inv-modal-backdrop" onClick={onClose}>
            <div className="channel-inv-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="channel-inv-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`channel-inv-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="channel-inv-chat-msg ai">…</div>
                    )}
                </div>
                <div className="channel-inv-chat-inp">
                    ...
                    <button type="button" className="channel-inv-btn channel-inv-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
