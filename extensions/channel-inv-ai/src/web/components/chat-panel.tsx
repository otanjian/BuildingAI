...
export function ChatPanel({
    ...
    return (
        <div className="channel-inv-card channel-inv-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--channel-inv-border)",
                    display: "flex",
            ...
            <div className="channel-inv-chat-msgs">
                {showOpening ? (
                    <div className="channel-inv-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="channel-inv-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="channel-inv-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="channel-inv-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`channel-inv-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`channel-inv-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="channel-inv-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
