...
describe("parseCheckResponse", () => {
    it("parses fenced JSON", () => {
        const text = `Analysis done.\n\`\`\`json\n{"ruleId":"CHI_001","anomalies":[]}\n\`\`\``;
        const result = parseCheckResponse(text, "CHI_001");
    ...
    it("fails on invalid JSON", () => {
        const result = parseCheckResponse("not json", "CHI_001");
    ...
    it("parses multiple anomalies", () => {
        ...
        const result = parseCheckResponse(JSON.stringify(payload), "CHI_002");
        expect(result.ok).toBe(true);
        if (result.ok) {
