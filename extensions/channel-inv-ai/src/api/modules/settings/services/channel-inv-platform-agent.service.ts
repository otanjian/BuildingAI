...
} from "../../../db/seed-data/channel-inv-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isChannelInvDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof CHI_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class ChannelInvPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: CHI_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: CHI_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: CHI_PLATFORM_AGENT_NAME,
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.CHI_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure CHI platform agent exists, register channel-inv-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: CHI_PLATFORM_AGENT_NAME } });
        const created = !agent;
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = CHI_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = CHI_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = CHI_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = CHI_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = CHI_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = CHI_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = CHI_PLATFORM_AGENT_MAX_STEPS;
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of CHI_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("channel-inv-mcp") ||
