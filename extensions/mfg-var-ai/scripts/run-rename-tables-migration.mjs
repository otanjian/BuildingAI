/**
 * Run MFGV 0.1.3 table rename migration (mfg-var-* prefix).
 *
 * Usage (from repo root, after `pnpm --filter mfg-var-ai build:api`):
 *   node extensions/mfg-var-ai/scripts/run-rename-tables-migration.mjs
 */
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'mfg_var_ai'
         ORDER BY table_name`,
    );
    console.log("mfg_var_ai tables:", tables.map((r) => r.table_name).join(", "));
} finally {
