/**
 * Fix duplicate mfg_var_ai tables: drop empty mfg-var-* stubs, rename legacy tables.
...
try {
    ...
    const tables = await dataSource.query(
        `SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'mfg_var_ai' ORDER BY table_name`,
    );
    console.log("mfg_var_ai tables:", tables.map((r) => r.table_name).join(", "));
    for (const { table_name } of tables) {
        const [{ c }] = await dataSource.query(
            `SELECT COUNT(*)::int AS c FROM "mfg_var_ai"."${table_name}"`,
        );
