...
import { MFGV_AI_SCHEMA, MFGV_TABLE } from "../../mfg-var-table-names";
import {
...
} from "../../seed-data/mfg-var-platform-agent.config";

export class MfgVarPlatformAgentSeeder extends BaseSeeder {
    readonly name = "MfgVarPlatformAgentSeeder";
    ...
    async run(dataSource: DataSource): Promise<void> {
        ...
        let agent = await agentRepo.findOne({ where: { name: MFGV_PLATFORM_AGENT_NAME } });
        if (agent) {
            ...
            await this.linkAgentToMfgVarSettings(dataSource, agent.id);
            return;
        }
        ...
        if (!owner) {
            this.logError("No user found; create a platform user before seeding MFGV agent");
            return;
        }
        ...
        this.logSuccess(`Created platform agent "${MFGV_PLATFORM_AGENT_NAME}" (${agent.id})`);
        this.logInfo(
    ...
    private async linkAgentToMfgVarSettings(dataSource: DataSource, agentId: string): Promise<void> {
        const qualified = `"${MFGV_AI_SCHEMA}"."${MFGV_TABLE.APP_SETTINGS}"`;
        ...
        this.logSuccess(
            `Linked agent to ${MFGV_AI_SCHEMA}.${MFGV_TABLE.APP_SETTINGS} (agent_id=${agentId})`,
        );
    }
}
