...
import {
    MFGV_ERP_MCP_NAME_HINTS,
    MFGV_PLATFORM_AGENT_AVATAR,
...
} from "../../../db/seed-data/mfg-var-platform-agent.config";


import { BowiMcpToolSyncService } from "../../bowi-mcp-tool-sync/bowi-mcp-tool-sync.service";
import { SettingsService } from "./settings.service";

export type AgentOptionRow = { id: string; name: string; isMfgVarDefault?: boolean };
...
export type AgentUpdatePreviewDto = {
    ...
    quickCommands: typeof MFGV_PLATFORM_AGENT_QUICK_COMMANDS;
    maxSteps: number;
    mcpServers: Array<{
        name: string;
        url: string;
...
@Injectable()
export class MfgVarPlatformAgentService {
    constructor(
    ...
    async listAgentOptions(userId: string): Promise<{
        ...
        const [mine, ehcsByName] = await Promise.all([
            ...
            this.agentRepo.findOne({ where: { name: MFGV_PLATFORM_AGENT_NAME } }),
        ]);
    ...
    async getAgentUpdatePreview(): Promise<AgentUpdatePreviewDto> {
        ...
        const agent = await this.agentRepo.findOne({ where: { name: MFGV_PLATFORM_AGENT_NAME } });
        ...
        const publishEmbedUrl =
            ...
            String(publishConfig.accessToken).trim()
                ...
                : null;
        return {
            agentName: MFGV_PLATFORM_AGENT_NAME,
            description: MFGV_PLATFORM_AGENT_DESCRIPTION,
            rolePrompt: MFGV_PLATFORM_AGENT_ROLE_PROMPT,
            openingStatement: MFGV_PLATFORM_AGENT_OPENING_STATEMENT,
            openingQuestions: MFGV_PLATFORM_AGENT_OPENING_QUESTIONS,
            quickCommands: MFGV_PLATFORM_AGENT_QUICK_COMMANDS,
            maxSteps: MFGV_PLATFORM_AGENT_MAX_STEPS,
            mcpServers: [
                {
    ...
    private buildPublishEmbedUrl(agentId: string, accessToken: string): string {
        const base =
            process.env.MFGV_MCP_BASE_URL?.trim() ||
    ...
    /**
     * Ensure MFGV platform agent exists, register mfg-var-mcp, and sync full profile.
     */
    async provisionForUser(userId: string): Promise<{ agentId: string; created: boolean }> {
        ...
        let agent = await this.agentRepo.findOne({ where: { name: MFGV_PLATFORM_AGENT_NAME } });
        const created = !agent;

        if (!agent) {
            agent = this.agentRepo.create({
                name: MFGV_PLATFORM_AGENT_NAME,
                description: MFGV_PLATFORM_AGENT_DESCRIPTION,
                createMode: "direct",
                avatar: MFGV_PLATFORM_AGENT_AVATAR,
                createBy: userId,
            });
    ...
    private applyAgentProfile(agent: Agent, modelId: string, mcpServerIds: string[]): void {
        agent.description = MFGV_PLATFORM_AGENT_DESCRIPTION;
        agent.avatar = MFGV_PLATFORM_AGENT_AVATAR;
        agent.rolePrompt = MFGV_PLATFORM_AGENT_ROLE_PROMPT;
        agent.openingStatement = MFGV_PLATFORM_AGENT_OPENING_STATEMENT;
        agent.openingQuestions = MFGV_PLATFORM_AGENT_OPENING_QUESTIONS;
        agent.quickCommands = MFGV_PLATFORM_AGENT_QUICK_COMMANDS;
        ...
        agent.maxSteps = MFGV_PLATFORM_AGENT_MAX_STEPS;
    ...
    private async ensureBowiMcpServer(userId: string): Promise<string> {
        ...
        if (toolCount === 0) {
            throw HttpErrorFactory.internalServerError("mfg-var-mcp tool catalog is empty");
        }
        return server.id;
    }
    ...
    private pickErpServer(servers: AiMcpServer[]): AiMcpServer | null {
        ...
        for (const hint of MFGV_ERP_MCP_NAME_HINTS) {
    ...
    private isBowiMcpServer(name: string, alias?: string | null): boolean {
        ...
        return (
            ...
            lower(name).includes("mfg-var-mcp") ||
