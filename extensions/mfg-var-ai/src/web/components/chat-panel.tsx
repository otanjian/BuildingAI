...
export function ChatPanel({
    ...
    return (
        <div className="mfg-var-card mfg-var-chat">
            <div
                style={{
                    padding: "10px 14px",
                    borderBottom: "1px solid var(--mfg-var-border)",
                    display: "flex",
            ...
            <div className="mfg-var-chat-msgs">
                {showOpening ? (
                    <div className="mfg-var-chat-opening">
                        {hasOpeningContent ? (
                            <EditorContentRenderer
                                value={openingStatement ?? ""}
                                className="mfg-var-chat-opening-text"
                            />
                        ) : null}
                        {openingQuestions.length > 0 ? (
                            <div className="mfg-var-chat-suggestions">
                                {openingQuestions.map((q) => (
                                    <button
                                        key={q}
                                        type="button"
                                        className="mfg-var-chat-suggestion"
                                        disabled={running}
                ...
                {lines.map((line) => (
                    <div key={line.id} className={`mfg-var-chat-msg ${line.role}`}>
                        {line.text}
                    </div>
                ))}
                {streamMessages.map((m) => {
                    ...
                    if (!text) return null;
                    return (
                        <div key={m.id} className={`mfg-var-chat-msg ${m.role === "user" ? "user" : "ai"}`}>
                            {text}
                        </div>
                    );
                })}
            </div>
            <div className="mfg-var-chat-inp">
                <input
                    value={input}
                    disabled={running}
                    placeholder="有问题，尽管问…"
