...
export function RcaModal({ anomaly, agentId, onClose }: Props) {
    ...
    return (
        <div className="mfg-var-modal-backdrop" onClick={onClose}>
            <div className="mfg-var-modal rca" onClick={(e) => e.stopPropagation()}>
                <h3 style={{ marginBottom: 12 }}>🧠 AI 根因分析 {anomaly.anomalyId}</h3>
                <div className="mfg-var-chat-msgs" style={{ minHeight: 280, maxHeight: 400 }}>
                    {chat.messages.map((m: { id: string; role: string }) => (
                        <div
                            key={m.id}
                            className={`mfg-var-chat-msg ${m.role === "user" ? "user" : "ai"}`}
                        >
                            {getTextFromMessage(m)}
                        </div>
                    ))}
                    {(chat.status === "streaming" || chat.status === "submitted") && (
                        <div className="mfg-var-chat-msg ai">…</div>
                    )}
                </div>
                <div className="mfg-var-chat-inp">
                    ...
                    <button type="button" className="mfg-var-btn mfg-var-btn-outline" onClick={onClose}>
                        关闭
                    </button>
                </div>
            </div>
