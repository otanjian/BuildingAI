...
import { MFGV_PLATFORM_AGENT_NAME } from "../../constants";
import { provisionMfgVarAgentViaPlatform } from "../../lib/provision-mfg-var-agent";
import {
...
export default function SettingsPage() {
    ...
    const hasMfgVarAgent = agentOptions.some((a) => a.name === MFGV_PLATFORM_AGENT_NAME);

    const handleProvision = useCallback(async () => {
        setProvisioning(true);
        try {
            const { agentId: id, created } = await provisionMfgVarAgentViaPlatform();
            setAgentId(id);
            toast.success(
                created
                    ? `已创建「${MFGV_PLATFORM_AGENT_NAME}」并注册 mfg-var-mcp`
                    : `已更新「${MFGV_PLATFORM_AGENT_NAME}」`,
            );
            await load();
        } catch (e) {
    ...
    useEffect(() => {
        if (loading || hasMfgVarAgent || autoProvisionStarted.current) {
            return;
        }
        autoProvisionStarted.current = true;
        void handleProvision();
    }, [loading, hasMfgVarAgent, handleProvision]);

    if (loading || (provisioning && !hasMfgVarAgent)) {
        return (
            <div className="mfg-var-card" style={{ padding: 20, maxWidth: 640 }}>
                <p style={{ fontSize: 13, color: "#64748b" }}>
                    {provisioning ? "正在更新 MFGV 智能体（mfg-var-mcp + 角色配置）…" : "加载中…"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="mfg-var-card" style={{ padding: 20, maxWidth: 640 }}>
            ...
            {!hasMfgVarAgent ? (
                ...
                >
                    <p style={{ marginBottom: 8 }}>未检测到「{MFGV_PLATFORM_AGENT_NAME}」智能体。</p>
                    <button
                        type="button"
                        className="mfg-var-btn mfg-var-btn-primary"
                        disabled={provisioning}
                        onClick={() => void handleProvision()}
                    >
                        {provisioning ? "更新中…" : "更新 MFGV 智能体"}
                    </button>
                </div>
            ) : (
                <p style={{ fontSize: 12, color: "#059669", marginBottom: 12 }}>
                    已检测到「{MFGV_PLATFORM_AGENT_NAME}」。可在平台
                    <strong> 智能体 → 我的智能体 </strong>
                    中查看；点击下方按钮可同步最新 PRD 配置与 mfg-var-mcp 工具。
                </p>
            )}

            <div className="mfg-var-form-row">
                <label>智能体</label>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4, marginBottom: 20 }}>
                ...
                <button
                    type="button"
                    className="mfg-var-btn mfg-var-btn-outline"
                    disabled={provisioning}
                    onClick={() => void handleProvision()}
                >
                    {provisioning ? "更新中…" : "更新 MFGV 智能体"}
