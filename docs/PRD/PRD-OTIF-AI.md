# 供应链 OTIF 自治系统（AI Agent 版）产品需求文档

**版本 V1.0.0（规划初稿）** · 产品需求 · 规划对齐 `extensions/otif-ai` 模板
...
| 项目 | 说明 |
|------|------|
| **是什么** | BuildingAI 扩展 **otif-ai**：平台 **智能体 + MCP** 对 ERP 数据做规则检查、异常诊断与（可选）自动修复 |
| **四个页面** | 驾驶舱 · 监控规则 · 异常明细 · **设置**（绑定 供应链 OTIF 自治助手） |
| **主流程** | 配规则 → 智能体「开始巡检」→ **智能体工具**读规则表 + MCP 逐条检查 → JSON 落库 → 驾驶舱刷新 → 根因分析 |
| **入口** | 应用中心 `/apps/otif-ai`（iframe 加载 `/extension/otif-ai`）；扩展直连 `/extension/otif-ai` |
| **V1.1 亮点** | 异常 **根因分析** 模态框；顶栏 🤖 打开 **右侧可调宽 iframe 智能体栏**（平台 WebAPP 发布页） |
| **全量巡检** | 用户发送「开始巡检」→ 智能体通过统一 **bowi-mcp**（`appId: otif-ai`）编排 + **ERP MCP** 取数 |

**健康分公式（驾驶舱核心指标）：**

```
健康分 = 100 − (OTIF偏低×10 + 缺料停线×15 + 供应商延迟×6 + 低风险×2)
```
...
| 项目 | 内容 |
|------|------|
| 文档名称 | 供应链 OTIF 自治系统（OTIF-AI Agent）V1.0 产品需求文档 |
| 产品名称 | 供应链 OTIF 自治系统 |
| 版本号 | V1.0.0（规划初稿） |
| 编制人 | 产品经理 |
| 编制日期 | 2026-05-25 |
| 适用范围 | 基于 BuildingAI 平台，通过 AI Agent + MCP 实现供应链 OTIF 自治系统；规划对齐 `extensions/otif-ai` 模板 |

### 修订记录

| 版本 | 日期 | 说明 |
|------|------|------|
| V1.0.0 | 2026-05-25 | 供应链 OTIF 自治系统规划初稿（母版：ehcs-ai 扩展） |
...
| 角色 | 典型操作 |
|------|----------|
| 供应链 OTIF 自治专员 | 配规则、发起全量巡检、处理异常、看驾驶舱 |
| 财务 / 供应链业务 | 查看本域异常、理解根因与修复建议 |
| 系统管理员 | 配置 MCP、模型、自动修复策略 |

### 1.4 产品边界

**自动修复边界：** 可建议加急 PO；不自动改承诺交期

**V1.0 规划范围内（扩展 `otif-ai`））**

- 页面：驾驶舱、规则维护、异常明细、**设置**
- 能力：全量巡检（规则表驱动、逐条 Agent）、单条根因分析、平台智能体对话
- 交付：PostgreSQL 模式 `otif_ai`、Console API、扩展 Web UI、与平台 `/api/ai-agents` 流式对接

**V1.0 范围外**（见 [§十 版本规划](#十版本规划)）

- 定时巡检、修复审批流、多 ERP 适配器矩阵
- 智能体在对话内**自行调用**规则表 API（无 OTIF 专用 MCP 工具时由**应用编排**读规则表）
...
| 设置 | `/settings` | 绑定平台智能体「供应链 OTIF 自治助手」；同步角色/开场白/MCP |

> **UI 原型** `docs/UI-OTIF-AI.html` 为 V1.0 侧栏 + 70/30 分栏演示稿；**以本文 V1.1.2 与扩展实现为准**。实现采用 **顶栏导航** + **右侧可收起、可调宽智能体栏**。
...
| 平台壳 | `/apps/otif-ai` 下**不**显示主站右侧浮动 Bot（由扩展自管智能体） |
...
| 驾驶舱主题 | 模板 `supply-chain`；主图 `trendArea`；健康分文案「交付健康分」 |
| KPI 文案 | 规则总数、OTIF%、交付健康分、关键件缺料、巡检批次、RCA 会话数 |
...
数据接口：`GET /otif-ai/console/dashboard/overview`（聚合）；字段与聚合逻辑见 `docs/DB/DB-OTIF-AI.md` 与各表说明。
...
| 交付健康分 | `100 − (OTIF偏低×10 + 缺料停线×15 + 供应商延迟×6 + 低风险×2)`，仅计待解决；副文案：较昨日 Δ |
...
| 业务域异常分布 | `otif-check_rules.business_domain` × 待解决/已解决 |
| 规则异常 Top5 | `otif-check_results` 按 `rule_id` 计数 |
| 异常处理状态分布 | 按 `status` 三类 |
| 最近批次执行状态 | `otif-check_run_items` 完成/失败/待处理堆叠 |
...
须已开启 **WebAPP 发布**（设置页保存或「更新 供应链 OTIF 自治助手」会写入 `enableSite` + `accessToken`）。

| 能力 | 行为 |
|------|------|
| 入口 / 关闭 | 顶栏 🤖 切换；侧栏标题 **✕** 关闭；无右下角浮动按钮、无顶栏「AI Agent」文字标签 |
| 栏宽 | 默认 400px；拖左侧分隔条调整 280–900px，宽度写入 `localStorage`（键 `otif-agent-dock-width`） |
| 对话 UI | 完整使用平台 **PublishChatPage**（开场白、建议问题、工具调用、会话历史） |
| 全量巡检 | 在嵌入页发送「开始巡检」→ 见 §3.1.3（智能体经 **bowi-mcp**，`appId: otif-ai`） |
| 自由对话 | 与独立打开发布页一致 |
...
1. 智能体调用 **`bowi_start_full_check`**（`appId: "otif-ai"`）→ 扩展 API 从规则表读取 `enabled=true` 规则并 `POST /check-runs`  
...
**数据范围（智能体 `rolePrompt` + 服务端 SQL 校验）：** 每个应用仅允许其 PostgreSQL schema（如 `otif_ai`、`inv_opt_ai`）及该应用 6 张业务表；详见 `@buildingai/constants/shared/bowi-app-scopes`。

**智能体提示词约定：**

- 每次 bowi-mcp 调用 **必须** 带本应用 `appId`（本应用为 `otif-ai`）  
...
|------|-----|
| 路由 | `/settings` |
| 存储 | `otif_ai.otif-app_settings.agent_id` |

| 能力 | 说明 |
|------|------|
| 智能体绑定 | 选择或 **创建/更新** 平台智能体「供应链 OTIF 自治助手」 |
...
- 编辑：模态框回填原数据  
- 变更持久化至 `otif_ai.otif-check_rules`（Console API）  
...
```
otif-check_rules (1) ──< (N) otif-check_results

otif-check_runs (1) ──< (N) otif-check_run_items ──> rule_id
otif-app_settings (singleton): agent_id
otif-rca_sessions (optional): anomaly_id, conversation_id
```

**Schema：** PostgreSQL `otif_ai`（扩展隔离）。表名前缀 **`otif-`**，SQL 须双引号；详见 `docs/DB/DB-OTIF-AI.md`。

### 4.1 `otif-check_rules`（监控规则）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | 自增 PK | 内部 ID |
| rule_id | varchar(20) | 业务 ID，如 `OTIF_001` |
| business_domain | varchar(20) | 交付准时 / 齐套率 / 供应商绩效 / 关键物料 |
...
### 4.2 `otif-check_results`（检查结果 / 异常）
...
| resolved_at | datetime | 解决时间（已解决/ai自动修复时写入，见 `docs/DB/DB-OTIF-AI.md`） |
| create_time | datetime | 记录创建时间 |

**索引建议：** `anomaly_id` 唯一；`rule_id`、`status`、`check_time` 供列表与聚合。

> 完整列类型、状态枚举与迁移记录见 **`docs/DB/DB-OTIF-AI.md`**。
### 4.3 `otif-app_settings`（应用设置）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | 自增 PK | 固定单行 |
| agent_id | varchar(64) | 平台智能体 ID（`供应链 OTIF 自治助手`） |
| update_time | datetime | 更新时间 |

### 4.4 `otif-check_runs` / `otif-check_run_items`（批次检查）

| 表 | 说明 |
|----|------|
| `otif-check_runs` | 一次全量巡检：`running` / `completed` / `cancelled` |
| `otif-check_run_items` | 每条规则状态：`pending` / `done` / `failed`，可存 `conversation_id` |
...
| 读规则 + 开批次 | 工具 `otif_start_full_check`（内部：`RulesService.listEnabled` + `CheckRunsService.start`） |
| 查看进度 | 工具 `otif_get_check_progress`（内部：`CheckRunsService.getProgress`） |
| 取消检查 | 工具 `otif_cancel_check`（内部：`CheckRunsService.cancelActive`） |
| ERP 取数 | 智能体已绑定的 **MCP**（非 OTIF 内置工具） |
| 单条落库 | 工具 `otif_ingest_rule_result`（内部：`CheckRunsService.ingest`） |

智能体角色提示词、开场白、建议问题模板见 `extensions/otif-ai/src/api/db/seed-data/otif-platform-agent.config.ts`。  
  ...
  "anomalies": [
    {
      ...
      "description": "供应链 OTIF 自治业务记录不满足规则约束",
      "riskLevel": "中",
...
- [ ] 设置页可创建/更新「供应链 OTIF 自治助手」并写入 `agent_id`
...
|---------|--------|----------|----------|------|------|----------|------|
| OTIF_001 | 交付准时 | 核心主数据 | 必填字段完整且格式符合规范 | 10 | 高 | 否 | 是 |
| OTIF_002 | 交付准时 | 业务单据 | 关键业务日期不得为空 | 10 | 中 | 否 | 是 |
| OTIF_003 | 齐套率 | 金额与数量 | 金额>0 且计量单位有效 | 15 | 高 | 否 | 是 |
| OTIF_004 | 齐套率 | 审批链 | 超过阈值单据须存在审批记录 | 12 | 中 | 否 | 是 |
| OTIF_005 | 供应商绩效 | 跨模块一致性 | 上下游单据数量/金额容差在阈值内 | 8 | 中 | 否 | 是 |
| OTIF_006 | 供应商绩效 | 主从关联 | 子记录必须关联有效父记录 | 8 | 中 | 否 | 是 |
| OTIF_007 | 关键物料 | 时效性 | 超期未处理记录须标记风险 | 10 | 高 | 否 | 是 |
| OTIF_008 | 关键物料 | 重复检测 | 同一业务键不得重复建档 | 5 | 低 | 是 | 是 |

### 9.2 异常结果

| 异常 ID | 规则 ID | 描述 | 风险 | 根因 | 方案 | 状态 |
|---------|---------|------|------|------|------|------|
| OTIF-20260525-001 | OTIF_001 | 供应链 OTIF 自治示例异常：规则触发 | 中 | 业务数据不满足规则约束 | 按建议方案整改 | 待解决 |
| OTIF-20260525-002 | OTIF_008 | 供应链 OTIF 自治示例异常：低危可自动修复 | 低 | 历史数据格式不一致 | Agent 自动修复演示 | ai自动修复 |
...
| 版本 | 交付范围 |
|------|----------|
| **V1.0（规划）** | 扩展 `otif-ai` 四页 + 开始巡检 + RCA |
...
| 健康分 | `100 − (OTIF偏低×10 + 缺料停线×15 + 供应商延迟×6 + 低风险×2)`（仅计待解决异常） |
| 自动修复 | 规则允许且 Agent 经 MCP 写回 ERP（见 §1.4） |
| 根因分析 | 针对单条异常的多步交互推理诊断 |
| OTIF | On-Time In-Full 交付绩效 |
...
| Supplier Scorecard | MCP 取数 / 校验 / 可选写回 |

---

*文档结束 — OTIF-AI PRD V1.0.0*
